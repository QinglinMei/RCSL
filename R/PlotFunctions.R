...
"PlotMST" <- function(drData, clustRes, TrueLabel, dataName = "",
  ...
  for(i in 1:centers_num){
      centers[,i] <- rowMeans(drData[,which(clustRes == i),drop = F])
  }

  centersSim <- (stats::cor(centers, method = "kendall") + 1) / 2
  g <- igraph::graph_from_adjacency_matrix(centersSim,mode = "undirected",weighted = T,diag = FALSE)
  g_mst <- igraph::mst(g)
  ...
  if(VisualMethod == "pca"){
     ...
     reducedData <- data.frame(res_PCA[,1:2])
...
"getLineage" <- function(drData, clustRes, pseudoTime, simMeasure = "kendall"){
  ...
  for(j in 1:max(clustRes)){
     centersPseudo[j] <- mean(pseudoTime[which(clustRes == j),drop = F])
  }
  g <- igraph::graph_from_adjacency_matrix(centersSim,mode = "undirected",weighted = T,diag = FALSE)
  g_mst <- igraph::mst(g)
  lineage <- numeric()
  ...
  if(LineageNum == 1){
     ...
     AdjacentNodes = as.list(igraph::adjacent_vertices(g_mst,v = c(1:centers_num)))
     for(i in 1:(centers_num - 2)){
     ...
     for(x in 1:length(vers)){
         EdgesRest[x,] <- Edges[which(Edges == vers[x], arr.ind = T)[1],]
      }
...
#' @export
#'
"PlotTrajectory" <- function(gfData, clustRes, lineage, TrueLabel,
  ...
  reducedData$label <- TrueLabel
  reducedDataCenters <- as.data.frame(reducedDataCenters[lineage,,drop = F])
  plot(0)
  ps <- data.frame(xspline(reducedDataCenters, shape = -0.2, lwd = 2, draw = F))
  dev.off()
...
"TrajectoryAnalysis" <- function(gfData, drData, S, clustRes, fontSize = 12, TrueLabel,
  ...
  TrajectoryPlot <- PlotTrajectory(gfData, clustRes, lineage, TrueLabel, fontSize,
